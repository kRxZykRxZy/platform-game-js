...
export default class Player extends Phaser.Physics.Arcade.Sprite {
  ...
  update(time) {
    ...
    if (time < this.dashUntil) { this.setVelocityX(this.facing * this.dashSpeed); this.setVelocityY(0); return; }
    let axis = 0;
    if (left) axis -= 1;
    if (right) axis += 1;
    if (axis !== 0) {
      ...
      this.setVelocityX(axis * this.moveSpeed * (grounded ? 1 : this.airControl));
    } else if (grounded) this.setVelocityX(0);
    if (jumpPressed) this.tryJump(grounded, time);
    if (this.y > this.scene.scale.height + 250) this.scene.handlePlayerDeath();
  }

  tryJump(grounded, time) {
    if (grounded || time - this.lastGrounded < 110) { this.setVelocityY(-this.jumpVelocity); this.jumpsUsed = 1; return; }
    if (this.jumpsUsed < 2) { this.setVelocityY(-this.doubleJumpVelocity); this.jumpsUsed = 2; }
  }
  ...
  dash(time) {
    ...
    this.setVelocity(this.facing * this.dashSpeed, 0);
  ...
  takeDamage(amount, time, sourceX) {
    ...
    this.setVelocity((this.x >= sourceX ? 1 : -1) * 330, -260);
